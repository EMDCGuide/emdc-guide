...
abstract class SettingsViewAbstract {
	...
	public function renderOptionControls($name, array $option = array()) {
		...
		switch ($option['type']) {
			...
			case Settings::TYPE_CUSTOM:
				return (is_callable($option['content']) ? call_user_func($option['content'], $name, $option) : $option['content']);
	...
	protected function renderExtraFields($name) {
		...
		$template = '<div class="cmreg-extra-field">
			...
			<input type="text" name="%s[%d][%s]" value="%s" placeholder="User meta name" required pattern="[a-z0-9_]+" title="Only lower-case basic latin characters, digits and underscore" /><br />
		...
		$out .= sprintf($template,
			esc_attr($name), 0, 'label', 'temp_val',
			esc_attr($name), 0, 'meta_name', 'temp_val',
