 ...
 */
jQuery(function($) {
  if ($('#copr-community-profile').length > 0) {
    $('.copr-js-hide').hide();
    $('.copr-js-show').show();
    $('.copr-my-response-textarea').val('');
    /**
    ...
    $(document).on('click', '.copr-edit-link', function() {
      ...
      if (parent.hasClass('is-editing')) {
        parent.find('.copr-answer-text-wrapper').first().show();
        ...
        parent.find('.copr-answer-text-wrapper').first().hide();
