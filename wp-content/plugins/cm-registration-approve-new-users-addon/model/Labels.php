...
class Labels extends Model {
	...
	const TEXT_DOMAIN = 'cm-registration-approve-new-users';
	...
	public static function init() {
		...
		add_action('cmreganu_load_label_file', array(__CLASS__, 'loadLabelFile'), 1);
		
		static::loadLabelFile();
		do_action('cmreganu_labels_init');
		
		/* You can use the following filters to add new labels for plugin:
		add_filter('cmreganu_labels_init_labels', function($labels) {
			$labels['label_name'] = array('default' => 'Value', 'desc' => 'Description', 'category' => 'Other');
			return $labels;
		});
		add_filter('cmreganu_labels_init_labels_by_categories', function($labelsByCategories) {
			$labelsByCategories['Other'][] = 'label_name';
			return $labelsByCategories;
		});
		*/
		
		static::$labels = apply_filters('cmreganu_labels_init_labels', static::$labels);
		static::$labelsByCategories = apply_filters('cmreganu_labels_init_labels_by_categories', static::$labelsByCategories);
