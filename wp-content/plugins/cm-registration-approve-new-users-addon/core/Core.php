...
namespace com\cminds\registration\addon\approvenewusers\core;
...
abstract class Core {
	...
	static function isLicenseOk() {
		global $cmreganu_isLicenseOk;
		return (!static::isPro() OR $cmreganu_isLicenseOk);
	}
	
	static function getPluginName($full = false) {
		return static::PLUGIN_NAME . (($full && static::isPro()) ? '' : '');
	}
