...
abstract class TaxonomyTerm extends Model {
	 ...
	 * @return com\cminds\registration\addon\approvenewusers\model\TaxonomyTerm
	 */
...
// 		return get_term_link($this->term, self::TAXONOMY);
	...
	function getTermMeta($key = '', $single = true) {
		return get_term_meta($this->getId(), $key, $single);
	}
	
	
	function setTermMeta($meta_key, $meta_value, $prev_value = '') {
		return update_term_meta($this->getId(), $meta_key, $meta_value, $prev_value);
	}
	
	
	function addTermMeta($meta_key, $meta_value, $unique = false) {
