...
class APSW_Search_Template {
	...
	public function get_search_query( $text ){
		
		if( isset( $_REQUEST['s'] ) && $_REQUEST['s'] != "" && $this->option_search_from['__search_by']  == 'tags' ) {
			return esc_html( $_REQUEST['s'] );
		}else{
			return $text;
		}
	}
	...
	public function  apsw_replace_woocomerce_search($query) {
		...
		if($query->is_search() &&  ! is_admin() && $query->is_main_query()  && ( is_product_category() || is_shop() )) {
	...
	public function apsw_get_woo_search_result(){
		...
		if(  $this->option_search_from['__search_by']  == 'tags' ){
		...
		if ( $the_query->have_posts() ) {
			...
			while ( $the_query->have_posts() ) : $the_query->the_post();
					...
					$search_results[] = array(
						...
						'category' 	=> ( apply_filters( 'apws_show_category', $this->option_search_results['show_category'] ) == 'yes' )? wp_strip_all_tags( wc_get_product_category_list( $product->get_id(), ', ') ) : '' ,
						...
						'add_to_cart_link' =>  ( apply_filters( 'apws_show_add_to_cart', $this->option_search_results['show_add_to_cart']  ) == 'yes' ) ? $product->add_to_cart_url() : '' ,
						
						'add_to_cart_text' => ( apply_filters( 'apws_show_add_to_cart', $this->option_search_results['show_add_to_cart']  ) == 'yes' ) ?  $product->add_to_cart_text(): '' ,
	...
	function apsw_search_shortcode($atts) {
		...
		ob_start();
		$this->apsw_search_style_common( absint( $style ) );
...
';
	...
	public function apws_search_element_category() {
		...
		$current_cat = ( isset( $_GET['category'] ) && $_GET['category'] != "" ) ? sanitize_text_field( $_GET['category'] ) : '';
			...
			<option value="0">'.esc_html( $this->option_search_from['all_category_text']  ).'</option>';
