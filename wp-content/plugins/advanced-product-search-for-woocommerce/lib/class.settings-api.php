 ...
 */
if ( !class_exists( 'WeDevs_Settings_API_Search' ) ):
class WeDevs_Settings_API_Search {
    ...
    function callback_license( $args ) {
		$license_options = get_option('apsw_license');
		
		if( !empty( $license_options ) && isset( $license_options['apsw_license_key'] ) ) :
		
		
			if( !empty( get_option('apsw_license_active') ) && $license_options['apsw_license_key'] ==  get_option('apsw_license_active') ) :
			...
			echo '<span style="color:#090">ACTIVATE </span>';	
				 ...
				 $request = wp_remote_get( 'https://athemeart.com/?ipn_license='.trim( $license_options['apsw_license_key'] ).'&client_siteurl='.get_site_url().'&theme=apsw-lang' );
					...
					if( ! empty( $data ) ) {
						update_option('apsw_license_active', $license_options['apsw_license_key'] );
