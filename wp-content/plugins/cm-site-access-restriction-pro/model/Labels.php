...
class Labels extends Model {
	...
	const OPTION_LABEL_PREFIX = 'cmacc_label_';
	const TEXT_DOMAIN = 'cm-site-access-restriction';
	...
	public static function init() {
		...
		add_action('cmacc_load_label_file', array(__CLASS__, 'loadLabelFile'), 1);
		
		static::loadLabelFile();
		do_action('cmacc_labels_init');
		
		/* You can use the following filters to add new labels for plugin:
		add_filter('cmacc_labels_init_labels', function($labels) {
			$labels['label_name'] = array('default' => 'Value', 'desc' => 'Description', 'category' => 'Other');
			return $labels;
		});
		add_filter('cmacc_labels_init_labels_by_categories', function($labelsByCategories) {
			$labelsByCategories['Other'][] = 'label_name';
			return $labelsByCategories;
		});
		*/
		
		static::$labels = apply_filters('cmacc_labels_init_labels', static::$labels);
		static::$labelsByCategories = apply_filters('cmacc_labels_init_labels_by_categories', static::$labelsByCategories);
		...
		foreach ($file as $row) {
			...
			if (count($row) >= 2) {
				...
				if(function_exists('pll_register_string')) {
					pll_register_string($row[1], $row[1], static::TEXT_DOMAIN);
				}
