...
abstract class SettingsAbstract {
	...
	public static function processPostRequest($data) {
		...
		do_action('cmacc_settings_after_save', $data);
    ...
    public static function getUsersNicknames($users){
        ...
        $result_users = get_users(array(
            'include' => $users
        ));

        if (count($result_users) === 0) return $result;
	...
	public static function getRolesOptionsExceptAdmin() {
		global $wp_roles;
		$result = array();
	...
	public static function getRestrictType() {
		$result = array(
			'full' => 'Restrict full content',
			'excerpt' => 'Restrict partially content',
			'shortcode' => 'Restrict content with shortcode only',
		);
		return $result;
	}
