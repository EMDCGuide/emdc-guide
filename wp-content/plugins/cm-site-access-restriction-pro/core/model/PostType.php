...
abstract class PostType extends Model {
	 ...
	 * @return com\cminds\siteaccessrestriction\model\PostType
	 */
	...
	function updateMultiplePostMeta($name, array $values, array $current) {
		$toDelete = array_diff($current, $values);
		$toAdd = array_diff($values, $current);
		foreach ($toDelete as $meta_value) {
			delete_post_meta($this->getId(), $name, $meta_value);
		}
		foreach ($toAdd as $meta_value) {
			add_post_meta($this->getId(), $name, $meta_value, $unique = false);
		}
		return $this;
	}
